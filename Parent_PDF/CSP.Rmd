...
To start, we need to specify the local and remote locations for storing project input and output data. The real study dataset (for study implementation and reproduction) can be read from a local location and fake data (for algorithm testing and demonstration) can be read from either local or remote (ie online repository) locations. Analyses outputs will be shared via online repositories. Different repositories will be used for outputs created with real or fake data and to map to AQOL-6D or CHU-9D health utility.
...
projects_ls <- list(AQOL_r4 = TTUProject(a_ScorzProfile = ScorzAqol6Adol(a_YouthvarsProfile = A,
                    ...
                    CHU_r4 = TTUProject(a_ScorzProfile = ScorzProfile(a_YouthvarsProfile = A,
                                                                     ...
                                                                     #instrument_version_1L_chr = XX,
                                                                     instrument_short_nm_1L_chr = "CHU-9D",
                                                                     itm_labels_chr = c("worry", "sadness", "pain", "tiredness", "annoyance", "school", 
                                                                                        "sleep", "daily routine", "activities"),
                                                                     itm_prefix_1L_chr = "chu9_",
                                                                     total_wtd_var_nm_1L_chr = "CHU9D"#, total_unwtd_var_nm_1L_chr = "chu9d_total_c"
                                                                     )))
rm(A) # Delete A - no longer needed
```

## Verify health utility scores
Our source dataset includes total weighted utility scores for both utility instruments. We can verify the calculations for one of these instruments, the adolescent AQoL-6D, by comparing the calculations in our dataset to those generated from item responses using the `ScorzAqol6Adol`.

First we apply the `ScorzAqol6Adol` scoring algorithm.

```{r}
#projects_ls$AQOL_r4<- renewSlot(projects_ls$AQOL_r4, "a_ScorzProfile") # Will replace subsequent line
projects_ls$AQOL_r4@a_ScorzProfile <- renew_ScorzAqol6Adol(projects_ls$AQOL_r4@a_ScorzProfile) # This is temporary
```

Next we compare the newly generated scores to those in our source dataset.

```{r}
projects_ls$AQOL_r4 <- renewSlot(projects_ls$AQOL_r4,
                                 "a_ScorzProfile@a_YouthvarsProfile@a_Ready4useDyad@ds_tb",
                                 new_val_xx = procureSlot(projects_ls$AQOL_r4, "a_ScorzProfile@a_YouthvarsProfile@a_Ready4useDyad@ds_tb") %>%
                                   dplyr::mutate(difference_aqol_calcs = AQOL6D - validation_aqol_w))
```

```{r}
# To manuall inspect the validation statistic, uncomment and run the following two lines
...
# A <- renewSlot(A, "b_SpecificParameters", 
...
predictors_r3 <- Ready4useRepos(dv_nm_1L_chr = "TTU",
                    ...
                    dv_server_1L_chr = "dataverse.harvard.edu") %>%
  ingest(fls_to_ingest_chr = c("predictors_r3"), metadata_1L_lgl = F)
projects_ls <- projects_ls %>% {
...
```{r}
A <-  renewSlot(A, "b_SpecificParameters@fake_1L_lgl", T#!is_real_1L_lgl
                ) 
```
```{r}
                 ...
                 renewSlot("ethics_1L_chr", 
                           paste0("The study was reviewed and granted approval by ", 
                                  ifelse(A@b_SpecificParameters@fake_1L_lgl,"no-one.",
                                         "the University of Melbourne’s Human Research Ethics Committee and the local Human Ethics and Advisory Group (ADD NUMBER HERE"))) %>%
