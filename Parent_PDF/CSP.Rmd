...
We update the dataset with CHU-9D data so that it includes the AQoL totals that we calculated in the previous step.
...
We begin by re-organising some of the metadata we have already specified about our dataset.
...
For each utility instrument, we next compare the performance of different model types. This step saves model objects and plots to a sub-directory of our output directory.
