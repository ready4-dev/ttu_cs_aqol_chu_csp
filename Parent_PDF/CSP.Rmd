...
```{r}
# You can inspect the dummy variables that will be created for each categorical candidate covariate by uncommenting and running:
# manufacture_Ready4useDyad(projects_ls$CHU_r4@c_SpecificProject@a_YouthvarsProfile@a_Ready4useDyad, type_1L_chr = "dummys", what_1L_chr = "factors", restrict_to_chr = projects_ls$CHU_r4@c_SpecificProject@b_SpecificParameters@candidate_covars_chr) 
```

We begin by renaming (for brevity) some of the default values for the dummy variables that will be created when using categorical covariates.

```{r}
projects_ls <- projects_ls %>% 
  purrr::map(~ renewSlot(.x, "c_SpecificProject", 
                         #investigateSlot(A,"c_SpecificProject")
                         renew_SpecificProject(.x@c_SpecificProject, new_val_xx = list(cpdiaggrouped = c("NotApp","Other","SubstanceUse"),
...
```{r}
projects_ls <- projects_ls %>% 
  purrr::map(~ renewSlot(.x, "c_SpecificProject@b_SpecificParameters@control_ls", list(adapt_delta = 0.99))) # This step may not be necessary when using the real data - skipping it will speed execution of the next step.
```
...
```{r }
projects_ls <- projects_ls %>% 
  purrr::map(~ renewSlot(.x, "c_SpecificProject", investigate_SpecificMixed(.x@c_SpecificProject, combinations_1L_lgl = T, 
                                                                            cores_1L_int = parallel::detectCores(), # For PC, use cores_1L_int = 1L
                                                                            max_nbr_of_covars_1L_int = 2L, depnt_var_min_val_1L_dbl = 0.00001))) ##investigateSlot(.x, "c_SpecificProject")
