...
```{r eval=FALSE}
is_real_1L_lgl <- F # 
path_to_data_1L_chr <- "****Replace with path to local data****"
...
projects_ls$CHU_r4 <- renewSlot(projects_ls$CHU_r4,
                                ...
                                new_val_xx = procureSlot(projects_ls$AQOL_r4, 
                                                         "a_ScorzProfile@a_YouthvarsProfile@a_Ready4useDyad@ds_tb"))
```
...
We ingest from one or our online repositories a table describing the candidate predictors that we will explore.

```{r}
predictors_r3 <- repositories_ls$aqol_real_r4 %>% # Make DS Specific
  ingest(fls_to_ingest_chr = c("predictors_r3"), metadata_1L_lgl = F)
projects_ls <- projects_ls %>% 
  purrr::map2(c("AQOL6D", "CHU9D"),
              ~ renewSlot(.x, 
                         "b_SpecificParameters@predictors_lup",
                         predictors_r3 %>% dplyr::filter(short_name_chr != .y))) %>% 
  stats::setNames(names(projects_ls))
rm(predictors_r3)
```
  ...
  purrr::map2(c("AQOL","CHU"),
              ~ {
                ready4::write_new_dirs(paste0(path_to_output_1L_chr,"/",.y),
                                       consent_1L_chr = "Y")
                 renewSlot(.x, "b_SpecificParameters@fake_1L_lgl", !is_real_1L_lgl) %>%
                   renewSlot("c_SpecificProject", 
                             SpecificModels(a_YouthvarsProfile = .x@a_ScorzProfile@a_YouthvarsProfile,
                                            b_SpecificParameters = .x@b_SpecificParameters,
                                            paths_chr = paste0(path_to_output_1L_chr,"/",.y)))
              }) %>% stats::setNames(names(projects_ls))
```

We next confirm that our parameters and dataset are internally consistent. This step will also rename dataset variables that use certain incompatible naming conventions.
